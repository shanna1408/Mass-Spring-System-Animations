...
mat4f TurnTableCamera::viewMatrix() const {
    vec3f camera_position_local{
        angle_to_x(zoom(), longitude(), latitude()),
        angle_to_y(zoom(), latitude()),
        angle_to_z(zoom(), longitude(), latitude())
    };
    vec3f up{ 0., 1., 0. };
    vec3f binormal = cross(camera_position_local, up);
    up = glm::normalize(cross(binormal, camera_position_local));

    vec3f camera_position = camera_position_local + translation();
